...
class readIDm:
    ...
    #
    @classmethod
    def add_show_parser(self, parser):
        pass

    @classmethod
    def add_dump_parser(self, parser):
        parser.add_argument(
    ...
    def add_format_parser(self, parser):
        ...
        readIDm.add_format_tt1_parser(subparsers.add_parser(
            'tt1', description=description))
        readIDm.add_format_tt2_parser(subparsers.add_parser(
            'tt2', description=description))
        readIDm.add_format_tt3_parser(subparsers.add_parser(
            'tt3', description=description))

    @classmethod
    def add_format_tt1_parser(self, parser):
        parser.add_argument(
            "--magic", metavar="BYTE", type=readIDm.parse_uint8,
            help="value to use as ndef magic byte")
        parser.add_argument(
            "--ver", metavar="x.y", type=readIDm.parse_version,
            help="ndef mapping major and minor version")
        parser.add_argument(
            "--tms", metavar="BYTE", type=readIDm.parse_uint8,
            help="tag memory size, 8*(tms+1)")
        parser.add_argument(
            "--rwa", metavar="BYTE", type=readIDm.parse_uint8,
            help="read write access byte")

    @classmethod
    def add_format_tt2_parser(self, parser):
        pass

    @classmethod
    def add_format_tt3_parser(self, parser):
        parser.add_argument(
            "--ver", metavar="x.y", type=readIDm.parse_version,
            help="ndef mapping major and minor version")
        parser.add_argument(
            "--nbr", metavar="BYTE", type=readIDm.parse_uint8,
            help="number of blocks that can be written at once")
        parser.add_argument(
            "--nbw", metavar="BYTE", type=readIDm.parse_uint8,
            help="number of blocks that can be read at once")
        parser.add_argument(
            "--max", metavar="SHORT", type=readIDm.parse_uint16,
            help="maximum number of blocks (nmaxb)")
        parser.add_argument(
            "--rfu", metavar="BYTE", type=readIDm.parse_uint8,
            help="value to set for reserved bytes")
        parser.add_argument(
            "--wf", metavar="BYTE", type=readIDm.parse_uint8,
            help="write-flag attribute value")
        parser.add_argument(
            "--rw", metavar="BYTE", type=readIDm.parse_uint8,
            help="read-write flag attribute value")
        parser.add_argument(
            "--len", metavar="INT", type=readIDm.parse_uint24,
            help="ndef length attribute value")
        parser.add_argument(
            "--crc", metavar="INT", type=readIDm.parse_uint16,
            help="checksum attribute value")

    @classmethod
    def add_emulate_parser(self, parser):
        ...
        readIDm.add_emulate_tt3_parser(subparsers.add_parser(
                'tt3', help='emulate a type 3 tag'))

    @classmethod
    def add_emulate_tt3_parser(self, parser):
        parser.add_argument(
        ...
        parser.add_argument(
            "--ver", metavar="x.y", type=readIDm.parse_version, default="1.0",
    ...
    @classmethod
    def add_protect_parser(self, parser):
        parser.add_argument(
            "-p", dest="password",
...
class TagTool(CommandLineInterface):
    def __init__(self):
        ...
        readIDm.add_show_parser(subparsers.add_parser(
                'show', help='pretty print ndef data'))
        readIDm.add_dump_parser(subparsers.add_parser(
                'dump', help='read ndef data from tag'))
        readIDm.add_load_parser(subparsers.add_parser(
                'load', help='write ndef data to tag'))
        readIDm.add_format_parser(subparsers.add_parser(
                'format', help='format ndef tag'))
        readIDm.add_protect_parser(subparsers.add_parser(
                'protect', help='write protect a tag'))
        readIDm.add_emulate_parser(subparsers.add_parser(
    ...
    def on_rdwr_startup(self, targets):
        if self.options.command in self.rdwr_commands.keys():
            #print("** カード読み込み待機中 **", file=sys.stderr)
            return targets
